...
TUNE = ROOT / "supra" / "tunes" / "supra 05292026 v3.xml.emub3"
VE1_EMUBT = ROOT / "supra" / "tunes" / "VE Table 1 05292026.emubt"
OUT_JSON = ROOT / "supra" / "tunes" / "ve2_lambda_delta_remap_spec.json"
OUT_REPORT = ROOT / "supra" / "tunes" / "VE2 lambda delta remap backcalc.md"
OUT_EMUBT = ROOT / "supra" / "tunes" / "VE Table 2 lambda-delta remap 05292026.emubt"
...
def main() -> None:
    ...
    ve1 = symbol(ve_xml, "veTable")
    ve_w = int(ve1["width"])
    ve_h = int(ve1["height"])
    ve_raw = parse_data(ve1)

    map_bins = parse_data(symbol(tune_xml, "mapBins"))
    rpm_bins = parse_data(symbol(tune_xml, "rpmBins"))
    lam_map_bins = parse_data(symbol(tune_xml, "mapBinsL8"))
    lam_rpm_bins = parse_data(symbol(tune_xml, "rpmBins10"))
    ...
    report = [
        ...
        "",
        f"Source VE1: `{VE1_EMUBT.relative_to(ROOT)}`",
        f"Lambda source: `{TUNE.relative_to(ROOT)}`",
        "",
        "Formula:",
        "",
        "```",
