...
> **Car-specific values live in the build working docs**, not here. For the reference build's chosen rate limits, blend points, and `idleActiveAirflow`/`idleDBWBlendPointTbl` values see [`supra/notes/airflow_actuator.md`](../supra/notes/airflow_actuator.md) and [`supra/notes/my_car.md`](../supra/notes/my_car.md). This note is intentionally car-agnostic.
...
My first instinct was to set it much lower at low speeds and much higher at high speeds. Setting the universal limit low enough to actually affect physical sensation interfered with DBW throttle body operation at idle — too aggressive a limit fights the idle controller's own plate movements.

### What Worked

A moderate universal limit, actually *reducing* the limit at high speed so lift-off behavior is gentler under high power, worked best. (See the build doc for this car's chosen value.)
...
Parking lot jerkiness was ultimately solved with a **scooped DBW characteristic in the low-PPS region**, while bumping up the characteristic in the upper-PPS region so throttle blips remain effective for downshifts.
...
- The armed state airflow table is the primary lever: hold higher airflow at high RPM with PPS = 0 to prevent an instant torque drop to zero. The trade-off is reduced engine braking feel. (How to set that table: [idle.md → Armed state airflow](idle.md).)
...
The idle→characteristic handoff (the `idleDBWBlendPointTbl` table, what releases at idle exit, and
why the blend point must track `idleActiveAirflow + Δ` across CLT) is now an idle-system settings
block: **[idle.md → DBW blend point](idle.md)**. In short: at exit, the airflow PID I-term, the
idle ignition correction, and the source switch all release at once, so a wound-negative PID dumps
a TPS + timing step (the tip-in bump) — keep the high-target `idleActiveAirflow` rows accurate and
re-derive the blend point whenever you rescale that table.