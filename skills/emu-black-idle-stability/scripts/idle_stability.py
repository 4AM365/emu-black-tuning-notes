...
# ---- config (thresholds; logs come from the command line) ---------------------
...
def main():
    ap = argparse.ArgumentParser(description="Idle target-tracking + stability (RPM) from EMU logs.")
    ap.add_argument("logs", nargs="+", help="EMU semicolon-delimited CSV log(s)")
    ...
    results = [r for r in (analyse(os.path.basename(p), p) for p in args.logs) if r and r["rows"]]
    if not results:
