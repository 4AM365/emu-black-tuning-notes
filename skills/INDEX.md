...
| [emu-black-emubp-export](emu-black-emubp-export/SKILL.md) | Pull a group of **scalar parameters** (cam/VVT setup, cam PID, overrun, idle/boost scalars) from any vehicle's XML tune into an importable `.emubp`. Template mode mirrors EMU's own `example … .emubp` group exports; resolves vehicle names via `EMU_SOURCE_INDEX.md`. Scalar sibling of emubt-export. | [pull_emubp.py](emu-black-emubp-export/scripts/pull_emubp.py) |
