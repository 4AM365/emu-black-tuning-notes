...
KCH = "Knock voltage peak cyl 6"    # overridable via --channel
RETARD = "Knock ign retard cyl 6"   # overridable via --retard
MAP_MIN = 130.0
...
NUM = ["TIME", "RPM", "MAP", KCH, "Knocking cylinders", RETARD]
...
def main():
    global KCH, RETARD, NUM
    ap = argparse.ArgumentParser(description="Combustion-CoV proxy from knock-voltage scatter (EMU logs).")
    ...
    ap.add_argument("--channel", default=KCH, help='knock-voltage channel (default cyl 6)')
    ap.add_argument("--retard", default=RETARD, help='knock-retard channel for the no-knock gate')
    ap.add_argument("--map-min", type=float, default=MAP_MIN, dest="map_min", help="boost MAP floor (kPa)")
    ap.add_argument("--plot", metavar="OUT.png", default=None, help="write a PNG (needs matplotlib)")
    args = ap.parse_args()
    KCH, RETARD = args.channel, args.retard
    NUM = ["TIME", "RPM", "MAP", KCH, "Knocking cylinders", RETARD]
    globals()["MAP_MIN"] = args.map_min
    GROUPS = {}
