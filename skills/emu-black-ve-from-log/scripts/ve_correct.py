"""
Correct veTable / veTable2 from a closed-loop EMU Black log.

Reads veTable / veTable2 (u12, scale 0.1) and their axis bins from the tune XML,
computes a per-cell multiplicative correction factor from steady-state closed-loop
samples, and writes a corrected copy of the tune.

The factor F = (1 + STFT/100) x (lambda_actual / lambda_target) folds in the trim the
ECU is already applying plus any residual error, accumulated bilinearly into the VE
grid and clamped. The SAME factor is applied to both veTable and veTable2: a closed-loop
dose error measured at one ethanol content corrects the *blended* result, so distributing
it to both keeps each table in play until validated on its own fuel (and preserves their
relative offset). NB the EMU VE table is a fuel-DOSE proxy, not pure air VE -- the two
tables legitimately diverge where the fuels want different mixtures; this is a first cut,
verify each on its own fuel afterward.

Usage:
  python ve_correct.py --tune TUNE.xml.emub3 --log LOG.csv --out CORRECTED.xml.emub3
...
_ap = argparse.ArgumentParser(description="Correct veTable/veTable2 from a closed-loop log.")
_ap.add_argument("--tune", required=True, help="tune XML (.xml.emub3) to correct")
_ap.add_argument("--log", required=True, help="closed-loop CSV log")
_ap.add_argument("--out", required=True, help="output corrected tune XML")
_ap.add_argument("--clamp", type=float, default=0.15, help="max +/- correction per pass")
_ap.add_argument("--weight-threshold", dest="wthr", type=float, default=25.0,
                 help="min accumulated bilinear weight to touch a cell (~1 s @ 25 Hz)")
_ap.add_argument("--acc-tol", dest="acctol", type=float, default=1.0,
                 help="|Acc. enrichment %%| below this = not a tip-in transient")
_args = _ap.parse_args()
TUNE, LOG, OUT = _args.tune, _args.log, _args.out
VE_SCALE = 0.1
WEIGHT_THRESHOLD = _args.wthr
CLAMP = _args.clamp
ACC_TOL = _args.acctol
